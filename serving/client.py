...
LIVE_RAW_DATA_PATH = os.path.join(os.path.dirname(_file_), 'live_raw')

class GameClient:
    def _init_(self, game_id: int):
    ...
    def get_reqd_df(self):
        ...
        with open(self.file_path, 'r+') as f:
            game = json.load(f)
            game_id, home_team_name, away_team_name, period, time_remaining, home_goals, away_goals, reqd_all_play_df =  get_features(game)
