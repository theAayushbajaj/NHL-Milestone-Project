...
def advanced_question3():
    ...
    #%%
    model.set_params(**best_hyperparams)
    X_train_top, X_val_top = feature_selection_question3(model, X_train, X_val, y_train, y_val)

    # Convert numpy arrays to pandas DataFrames
    X_train_top_df = pd.DataFrame(X_train_top)
    X_val_top_df = pd.DataFrame(X_val_top)

    # Concatenate using pd.concat
    X_combined = pd.concat([X_train_top_df, X_val_top_df])
    ...
    # Train the model with the best hyperparameters
    model.set_params(**best_hyperparams)
    model.fit(pd.concat([X_train_top, X_val_top]), pd.concat([y_train, y_val]))
