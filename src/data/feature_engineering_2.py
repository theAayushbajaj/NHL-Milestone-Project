...
def add_shot_distance_and_correct_coordinates(df_game):
    ...
    np_shot_distance = df_shot_distance.apply(lambda game: compute_shot_distance(
                                                    game['x_coordinate'], game['y_coordinate'], game['rinkSide']),
                                                    axis=1)
    ...
    if(np.mean(inverted_shot_distance) < np.mean(np_shot_distance)):
        df_shot_distance['shot_distance'] = inverted_shot_distance
        ...
        df_shot_distance['shot_distance'] = np_shot_distance
...
def add_game_shot_angle(df_game):
    ...
    df_add_shot_angle['shot_angle'] = df_add_shot_angle.apply(lambda game: compute_shot_angle(
                                                                game['x_coordinate'], game['y_coordinate'], game['rinkSide']),
                                                                axis=1)
...
def add_distance_from_last_event_and_speed(df_game):
    ...
    df_add_speed_distance_features['distance_from_last_event'] = df_add_speed_distance_features.apply(lambda game: compute_distance_previous_current_events(
                                                                game['x_coordinate'], game['y_coordinate'], game['x_last_event'],
                                                                game['y_last_event']), axis=1)
    df_add_speed_distance_features['speed'] = df_add_speed_distance_features.apply(lambda game:compute_speed(
                                                                game['distance_from_last_event'], game['time_from_last_event']),
                                                                axis=1)
...
def add_change_in_game_shot_angle(df_game):
    ...
    df_change_in_shot_angle['change_in_shot_angle'] = df_change_in_shot_angle.apply(lambda game: compute_change_in_angle(
                                                                            game['is_rebound'], game['shot_angle'],
                                                                            game['y_coordinate'], game['x_last_event'],
                                                                            game['y_last_event'], game['rinkSide']), axis=1)
...
df_nhl_data = pd.read_csv("playoff_2020.csv")
df_nhl_data = feature_engg(df_nhl_data)
df_nhl_data['is_goal'] = df_nhl_data['is_goal'].map({True: 1, False: 0})
df_nhl_data['emptyNet'] = df_nhl_data['emptyNet'].map({True: 1, False: 0})
df_nhl_data.to_csv('playoff_2020_new.csv',index=False)
